...
def binom_gr_lik_opt(alpha, y, zM, k, ps_y, p_z_ys, nj):    
    ...
    theta_prime = np.full(r + 1, nj) * np.expand_dims(pi, axis = 1)
    #theta_prime[:,:,:,1] = theta_prime[:,:,:,1] * zM 
    
    So = np.expand_dims(y[..., np.newaxis, np.newaxis], axis = 0) - theta_prime
    So[:,:,:,1] = So[:,:,:,1] * zM 

    return - np.sum(ps_y[..., np.newaxis] * \
                    np.sum(p_z_ys[...,np.newaxis] *  So, axis = 0), axis = (0,1))
...
def categ_gr_lik(theta, y_oh, zM, k, o1, ps_y, p_z_ys):
    ''' Does not work. yg use and the place/signs of the terms is not very clear for me...
    '''
    r = zM.shape[1]
    M = zM.shape[0]
    numobs = len(y_oh)
    ...
    eta = broad_thro - zT @ alphao.reshape(r, 1, 1, 1)
    ...
    der_theta_alphao = - zT[np.newaxis] * gamma_next[1:] # A vérifier
    der_b_alphao = - zT[np.newaxis] * gamma # A vérifier
    
    # Format the derivatives
    der_prev_theta_thr = np.concatenate([np.zeros((1,M, k, r)), der_prev_theta_thr])
    der_prev_b_thr = np.concatenate([np.zeros((1,M, k, r)), der_prev_b_thr])
    ...
    return np.concatenate([thr_grad, np.atleast_2d(alphao_grad)]).T[0] # Hack to remove
